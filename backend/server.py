...
# CD31: 19, CD11b: 37, Catalase: 59, CD4: 25, CD20: 27
...
def generate_vitessce_config(view_state_data):
    ...
    if not view_state_data:
        ...
        # Handle case where no data is sent - maybe return a default?
        vc = VitessceConfig(schema_version="1.0.16", name="Error - No View State")
        return vc.to_dict() # Or return None/raise error
    ...
    try:
        ...
        if "imageLayer" in view_state_data and isinstance(view_state_data["imageLayer"], list):
            ...
            for layer_data in view_state_data["imageLayer"]:
                processed_layer = layer_data.copy() # Start with a copy
                ...
                if "imageChannel" in processed_layer and isinstance(processed_layer["imageChannel"], list):
                    # Directly use the list of channel dicts, wrap the list itself in CL
                    processed_layer["imageChannel"] = CL(processed_layer["imageChannel"])
                else:
                    # Handle case where imageChannel might be missing or not a list
                    processed_layer["imageChannel"] = CL([]) 
                processed_layers.append(processed_layer)
            # Wrap the list of processed layer dicts in CL
            reconstructed_state["imageLayer"] = CL(processed_layers)
        else:
             # Handle case where imageLayer is missing or not a list
            reconstructed_state["imageLayer"] = CL([])
        # --- End CL reconstruction --- 
...
@app.route('/api/test-zarr-structure')
def test_zarr_structure():
    """Opens the Zarr node at path /0 using generic open() and reports its type."""
    # Define the URL including the /0 path
    zarr_url = "https://lsp-public-data.s3.amazonaws.com/yapp-2023-3d-melanoma/Dataset1-LSP13626-melanoma-in-situ"
    logger.info(f"Attempting to open Zarr node at path: {zarr_url}")

    try:
        # Use FSStore
        store = pyzarr.storage.FSStore(zarr_url)
        # Use the generic open function
        zarr_node = pyzarr.open(store, mode='r') # Open in read-only mode

        if isinstance(zarr_node, pyzarr.hierarchy.Group):
            logger.info("Successfully opened as Zarr Group.")
            print("Structure inside /0 group:")
            print(zarr_node.tree(expand=True))
            return jsonify({"message": "Node at /0 opened as GROUP. Check Flask console for structure."}), 200
        elif isinstance(zarr_node, pyzarr.core.Array):
            logger.info("Successfully opened as Zarr Array.")
            print(f"Array Shape: {zarr_node.shape}") # Print shape
            print(f"Array Dtype: {zarr_node.dtype}") # Print dtype
            return jsonify({"message": "Node at /0 opened as ARRAY. Check Flask console for shape/dtype."}), 200
        else:
            logger.error(f"Opened node is neither Group nor Array? Type: {type(zarr_node)}")
            return jsonify({"error": f"Opened node at {zarr_url} is of unknown type: {type(zarr_node)}"}), 500

    except Exception as e:
        logger.error(f"Error opening Zarr node {zarr_url}: {e}", exc_info=True)
        # Also log the full traceback
        import traceback
        logger.error(traceback.format_exc())
        return jsonify({"error": f"Failed to open Zarr node: {e}"}), 500
# --- Add this new route to your Flask app ---
@app.route('/api/analyze_roi_markers', methods=['POST'])
def analyze_roi_markers():
    try:
        data = request.get_json()
        threshold_factor = float(data.get('threshold', 0.5)) # Use received threshold (e.g., 0.5 means 50% of max)
        radius = int(data.get('radius', 50))
        image_id = data.get('image_id', None) # Get optional image identifier

        print(f"Received analysis request: threshold_factor={threshold_factor}, radius={radius}, image_id={image_id}")

        # --- Image Analysis Steps ---
        # 0. Load data (Replace with your actual loading logic)
        marker_data = load_image_data(image_id)
        if 'CD31' not in marker_data:
             return jsonify({"error": "CD31 channel not found in loaded data."}), 400

        cd31_img = marker_data['CD31']
        if cd31_img.ndim != 3:
             return jsonify({"error": f"Expected 3D image for CD31, but got {cd31_img.ndim} dimensions."}), 400

        # 1. Compute CD31 projection over z (Max intensity projection)
        print("Computing CD31 Z-projection...")
        cd31_proj = np.max(cd31_img, axis=0)

        # 2. Threshold projection to get vessel mask
        # Using a factor of the max intensity as threshold
        threshold_value = cd31_proj.max() * threshold_factor
        print(f"Applying threshold: {threshold_value} ({threshold_factor*100}% of max)")
        vessel_mask = cd31_proj > threshold_value

        if not np.any(vessel_mask):
             return jsonify({"error": f"No vessels found above threshold {threshold_value}. Try lowering the threshold."}), 400

        # 3. Compute distance transform to vessel boundary
        # distance_transform_edt calculates distance to the *nearest zero* (background)
        # So, we calculate distance on the inverted mask to get distance *to* the vessels
        print("Computing distance transform...")
        distance_to_vessel = ndimage.distance_transform_edt(~vessel_mask)

        # 4. Select all pixels within the specified radius
        print(f"Selecting pixels within {radius} pixel radius...")
        near_vessel_mask = distance_to_vessel <= radius
        # Exclude pixels that are *part of* the vessel itself
        near_vessel_boundary_mask = near_vessel_mask & (~vessel_mask)

        if not np.any(near_vessel_boundary_mask):
             return jsonify({"error": f"No pixels found within {radius} pixels of vessels (excluding vessel pixels themselves)."}), 400

        # 5. Extract mean z-projected intensity for other markers in that region
        print("Calculating mean intensities in the region...")
        results = {}
        markers_to_analyze = ['CD20', 'CD11b', 'CD4', 'Catalase']

        for marker_name in markers_to_analyze:
            if marker_name in marker_data:
                marker_img = marker_data[marker_name]
                if marker_img.ndim == 3:
                    marker_proj = np.max(marker_img, axis=0) # Project if 3D
                elif marker_img.ndim == 2:
                     marker_proj = marker_img # Assume already projected if 2D
                else:
                    print(f"Warning: Skipping {marker_name} - expected 2D or 3D data, got {marker_img.ndim}D.")
                    continue

                # Calculate mean intensity ONLY in the 'near_vessel_boundary_mask'
                mean_intensity = np.mean(marker_proj[near_vessel_boundary_mask])
                results[marker_name] = float(mean_intensity) # Convert to standard float for JSON
                print(f"  Mean {marker_name}: {mean_intensity:.2f}")
            else:
                results[marker_name] = None # Indicate marker was not found
                print(f"  Warning: {marker_name} data not found.")

        # 6. Return results (Plotting happens in frontend)
...
def get_histogram_data():
    """Calculates and returns histogram data for a given channel and ROI."""
    logger.info("Request received for /api/histogram")
    data = request.get_json()
    if not data:
        logger.error("No JSON data received for histogram.")
    ...
    if channel_index is None:
        logger.error(f"Missing 'channel_index' in request: {data}")
        return jsonify({"error": "Missing 'channel_index'"}), 400
    ...
    if roi:
        if not all(k in roi for k in ('z', 'y', 'x')) or \
           not all(isinstance(roi[k], list) and len(roi[k]) == 2 for k in ('z', 'y', 'x')):
            logger.error(f"Invalid 'roi' format: {roi}. Expected {{'z': [s,e], 'y': [s,e], 'x': [s,e]}} or null")
        ...
        try:
            ...
            logger.info(f"Using ROI: {valid_roi}")
             ...
             logger.error(f"Invalid numeric value in ROI: {roi}")
             return jsonify({"error": "Invalid numeric value in ROI components."}), 400
    else:
        logger.info("No ROI provided, processing full channel.")

    logger.info(f"Histogram requested for Channel: {channel_index}, ROI: {roi}, Bins: {data.get('num_bins', 100)}")

    # --- Open the target Zarr array ---
    target_zarr_array = open_target_zarr_array()
    if target_zarr_array is None:
        # Error logged within open_target_zarr_array
        return jsonify({"error": "Failed to open target Zarr array on server."}), 500
    # ---

    try:
        # --- Extract ROI tuples OR Full Extent ---
        # This section determines the range if ROI is *not* provided by the user
        # Assuming 5D array shape like (T, C, Z, Y, X)
        roi_z = tuple(valid_roi['z']) if valid_roi and valid_roi.get('z') else (0, target_zarr_array.shape[2]) # Use Z dimension (index 2)
        roi_y = tuple(valid_roi['y']) if valid_roi and valid_roi.get('y') else (0, target_zarr_array.shape[3]) # Use Y dimension (index 3)
        roi_x = tuple(valid_roi['x']) if valid_roi and valid_roi.get('x') else (0, target_zarr_array.shape[4]) # Use X dimension (index 4)

        # --- Call the imported analysis function ---
        logger.info(f"Calling calculate_histogram with array shape {target_zarr_array.shape} using ROI Z:{roi_z}, Y:{roi_y}, X:{roi_x}...")
        hist_result = calculate_histogram(
            zarr_array=target_zarr_array,
            channel_index=channel_index,
            roi_z=roi_z, # Pass Z ROI
            roi_y=roi_y, # Pass Y ROI
            roi_x=roi_x, # Pass X ROI
            num_bins=data.get('num_bins', 100)
        )
        # ---

        # Check for errors from the analysis function
        if 'error' in hist_result:
            logger.error(f"Error from calculate_histogram: {hist_result['error']}")
            # Return a 500 error, as the calculation failed server-side
            return jsonify({"error": f"Histogram calculation failed: {hist_result['error']}"}), 500
        else:
            logger.info(f"✅ Histogram calculated successfully for channel {channel_index}.")
            return jsonify(hist_result), 200 # Return the {'counts': [...], 'bin_edges': [...]} dict

    except Exception as e:
        logger.error(f"❌ Unexpected error in /api/histogram route: {e}", exc_info=True)
        return jsonify({"error": f"Internal server error: {e}"}), 500
# --- End Histogram Endpoint ---
...
def find_top_interaction_points(heatmap, num_points=10, min_distance=50):
    """
    Find top N points in a heatmap ensuring minimum distance between points.
    Returns points with their normalized values.
    """
    if heatmap is None or not isinstance(heatmap, list) or not heatmap:
        return []
    
    # Convert to numpy array for easier processing
    heatmap_array = np.array(heatmap)
    
    # Apply threshold to ignore very small values (e.g., less than 0.1)
    threshold = 0.1
    heatmap_array[heatmap_array < threshold] = 0
    
    # Log statistics before processing
    logger.info(f"\nTop Points Statistics:")
    logger.info(f"  Total pixels: {heatmap_array.size}")
    logger.info(f"  Non-zero pixels: {np.sum(heatmap_array > 0)}")
    logger.info(f"  Values near 1 (>0.9): {np.sum(heatmap_array > 0.9)}")
    logger.info(f"  Values near 0 (<0.1): {np.sum(heatmap_array < 0.1)}")
    logger.info(f"  Mean value: {np.mean(heatmap_array[heatmap_array > 0]):.3f}")
    logger.info(f"  Max value: {np.max(heatmap_array):.3f}")
    logger.info(f"  Min value: {np.min(heatmap_array[heatmap_array > 0]):.3f}")
    
    # Flatten the heatmap and get all indices sorted by value
    flat_indices = []
    for i in range(len(heatmap_array)):
        for j in range(len(heatmap_array[0])):
            if heatmap_array[i][j] > 0:  # Only consider non-zero values
                flat_indices.append((i, j, heatmap_array[i][j]))
    ...
    # Log top 10 values before distance filtering
    logger.info("\nTop 10 values before distance filtering:")
    for i, (y, x, val) in enumerate(flat_indices[:10]):
        logger.info(f"  {i+1}. Position: ({x}, {y}), Value: {val:.3f}")
    
    # Scale factor for coordinates (changed from 4 to 8 to match resolution 3)
    SCALE_FACTOR = 8
    
    # Iterate through sorted indices
    for i, j, value in flat_indices:
        # Check if this point is far enough from all previously selected points
        is_far_enough = True
        for used_i, used_j in used_positions:
            distance = np.sqrt((i - used_i)**2 + (j - used_j)**2)
            if distance < min_distance:
                is_far_enough = False
                break
        
        if is_far_enough:
            # Scale coordinates by factor 8
            scaled_x = j * SCALE_FACTOR
            scaled_y = i * SCALE_FACTOR
            
            points.append({
                'x': scaled_x,
                'y': scaled_y,
                'value': float(value)  # Store original value
            })
    ...
    # Normalize values of selected points
    if points:
        # Get the maximum and minimum values among selected points
        ...
        for point in points:
            if max_value > min_value:
                ...
                normalized_value = (point['value'] - min_value) / (max_value - min_value)
            else:
                normalized_value = 1.0  # If all values are the same
            point['value'] = float(normalized_value)
    
    # Log final selected points with normalized values
    logger.info("\nFinal selected points with normalized values:")
    for i, point in enumerate(points):
        logger.info(f"  {i+1}. Position: ({point['x']}, {point['y']}), Normalized Value: {point['value']:.3f}")
    
    return points
# Global cache for top interaction points
TOP_POINTS_CACHE = {}
CACHE_FILE = 'top_points_cache.json'
def save_cache_to_file():
    """Save the cache to a JSON file"""
    try:
        if not TOP_POINTS_CACHE:
            logger.warning("No data in cache to save")
            return False

        # Convert numpy values to Python native types for JSON serialization
        cache_to_save = {}
        for group_key, points in TOP_POINTS_CACHE.items():
            cache_to_save[group_key] = [
                {
                    'x': int(point['x']),  # Already scaled by factor 4
                    'y': int(point['y']),  # Already scaled by factor 4
        ...
        logger.info(f"Cache contains data for groups: {list(cache_to_save.keys())}")
        return True
    except Exception as e:
        logger.error(f"Error saving cache to file: {str(e)}", exc_info=True)
        return False

def load_cache_from_file():
    """Load the cache from JSON file if it exists"""
    global TOP_POINTS_CACHE
    try:
        if os.path.exists(CACHE_FILE):
            with open(CACHE_FILE, 'r') as f:
                TOP_POINTS_CACHE = json.load(f)
            logger.info(f"Successfully loaded cache from {CACHE_FILE}")
            logger.info(f"Loaded data for groups: {list(TOP_POINTS_CACHE.keys())}")
            return True
        else:
            logger.info(f"Cache file {CACHE_FILE} does not exist yet")
            return False
    except Exception as e:
        logger.error(f"Error loading cache from file: {str(e)}", exc_info=True)
        return False

def get_roi_from_heatmap():
    """Get ROI from the Zarr array dimensions"""
    try:
        zarr_array = open_target_zarr_array()
        if zarr_array is None:
            return None
        ...
        z_dim, y_dim, x_dim = zarr_array.shape[2:]
        
        # Use full data range
        roi = {
            'z': [0, z_dim],  # Full Z range
            'y': [0, y_dim],  # Full Y range
            'x': [0, x_dim]   # Full X range
        }
        
        logger.info(f"Using full data range ROI: Z[0-{z_dim}], Y[0-{y_dim}], X[0-{x_dim}]")
        return roi
    except Exception as e:
        logger.error(f"Error getting ROI from heatmap: {str(e)}", exc_info=True)
        return None

def calculate_all_groups_top_points(num_points=20):
    """Calculate top points for all groups and cache them"""
    try:
        # Get ROI for full data range
        roi = get_roi_from_heatmap()
        if not roi:
            return {'status': 'error', 'message': 'Failed to get ROI from heatmap'}

        # Open the Zarr array
        zarr_array = open_target_zarr_array()
        if zarr_array is None:
            return {'status': 'error', 'message': 'Failed to open Zarr array'}

        logger.info(f"Calculating heatmaps for all groups using full data range...")
        
        # Calculate heatmap for all groups with timeout
        try:
            heatmap_data = calculate_rgb_interaction_heatmap_py(
                zarr_array,
                roi['z'],
                roi['y'],
                roi['x'],
            ...
            logger.error(f"Error calculating heatmaps: {str(e)}", exc_info=True)
            return {'status': 'error', 'message': f'Error calculating heatmaps: {str(e)}'}

        if 'error' in heatmap_data:
            return {'status': 'error', 'message': heatmap_data['error']}

        # Process each group with progress logging
        for group_id in range(1, 5):  # Groups 1-4
            try:
                group_key = f'group_{group_id}'
                if group_key in heatmap_data['heatmaps']:
                    logger.info(f"Processing group {group_id}...")
                    group_heatmap = heatmap_data['heatmaps'][group_key]
                    top_points = find_top_interaction_points(group_heatmap, num_points, min_distance=50)
                    TOP_POINTS_CACHE[group_key] = top_points
                    logger.info(f"Calculated {len(top_points)} top points for group {group_id}")
                else:
                    logger.warning(f"No heatmap data found for group {group_id}")
            except Exception as e:
                logger.error(f"Error processing group {group_id}: {str(e)}", exc_info=True)
        ...
        try:
            if save_cache_to_file():
                ...
                return {'status': 'error', 'message': 'Failed to save cache to file'}
        except Exception as e:
            logger.error(f"Error saving cache: {str(e)}", exc_info=True)
            return {'status': 'error', 'message': f'Error saving cache: {str(e)}'}
        
        return {
            'status': 'success',
            'message': f'Successfully calculated and cached top {num_points} points for all groups'
        }

    except Exception as e:
        logger.error(f"Error in calculate_all_groups_top_points: {str(e)}", exc_info=True)
        return {'status': 'error', 'message': str(e)}
@app.route('/api/calculate_all_top_points', methods=['POST'])
def calculate_all_top_points():
    """API endpoint to calculate and cache top points for all groups."""
    try:
        data = request.get_json()
        num_points = data.get('num_points', 20)
        
        result = calculate_all_groups_top_points(num_points)
        ...
        logger.error(f"Error in calculate_all_top_points endpoint: {str(e)}", exc_info=True)
        return jsonify({
            'status': 'error',
            'message': str(e)
        }), 500
@app.route('/api/top_interaction_points', methods=['POST'])
def get_top_interaction_points_api():
    """Get top interaction points for a specific group"""
    try:
        data = request.get_json()
        ...
        # Try to load cache from file if not already loaded
        if not TOP_POINTS_CACHE:
            load_cache_from_file()

        # Check if we have cached points
        if group_key in TOP_POINTS_CACHE:
            cached_points = TOP_POINTS_CACHE[group_key]
            if len(cached_points) >= num_points:
                logger.info(f"Returning {num_points} cached points for group {group_id}")
                return jsonify({'points': cached_points[:num_points]})

        # If no cache, calculate new points
        roi = get_roi_from_heatmap()
        if not roi:
            return jsonify({'error': 'Failed to get ROI from heatmap'}), 500
        ...
        logger.info(f"Calculating heatmap for ROI: {roi}")
        ...
        # Get top points
        if group_key not in heatmap_data['heatmaps']:
            return jsonify({'error': f'No heatmap data for group {group_id}'}), 500
        ...
        return jsonify({'points': top_points})

    except Exception as e:
        logger.error(f"Error in get_top_interaction_points_api: {str(e)}", exc_info=True)
        return jsonify({'error': str(e)}), 500

# --- New Endpoint for Saving Default Channels ---
@app.route('/api/save_default_channels', methods=['POST'])
def save_default_channels():
    """Save the default channels data in Zarr format."""
    try:
        logger.info("Starting save_default_channels function...")
        
        # Create Input directory if it doesn't exist
        input_dir = Path("Input")
        logger.info(f"Creating/checking Input directory at: {input_dir.absolute()}")
        try:
            input_dir.mkdir(exist_ok=True)
        except Exception as dir_error:
            error_msg = f"Failed to create Input directory: {str(dir_error)}"
            logger.error(error_msg)
            return jsonify({"error": error_msg}), 500
        
        # Get the target resolution array
        logger.info("Attempting to open target Zarr array...")
        try:
            target_zarr_array = open_target_zarr_array()
            if target_zarr_array is None:
                error_msg = "Failed to open Zarr array - open_target_zarr_array returned None"
                logger.error(error_msg)
                return jsonify({"error": error_msg}), 500
        except Exception as zarr_error:
            error_msg = f"Error opening Zarr array: {str(zarr_error)}"
            logger.error(error_msg)
            return jsonify({"error": error_msg}), 500
            
        logger.info(f"Successfully opened Zarr array with shape: {target_zarr_array.shape}")
            
        # Default channel indices and settings
        default_channels = {
            19: {"name": "CD31", "color": [0, 255, 0], "window": [300, 20000]},
            27: {"name": "CD20", "color": [255, 255, 0], "window": [1000, 7000]},
            37: {"name": "CD11b", "color": [255, 0, 255], "window": [700, 6000]},
            25: {"name": "CD4", "color": [0, 255, 255], "window": [1638, 10000]},
            59: {"name": "Catalase", "color": [255, 0, 0], "window": [1638, 7000]}
        }
        
        # Create timestamp for unique filename
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        output_filename = f"default_channels_{timestamp}.zarr"
        output_path = input_dir / output_filename
        logger.info(f"Will save to: {output_path}")
        
        # Create Zarr group
        logger.info("Creating Zarr group...")
        try:
            store = pyzarr.DirectoryStore(str(output_path))
            root = pyzarr.group(store=store, overwrite=True)
        except Exception as store_error:
            error_msg = f"Failed to create Zarr store: {str(store_error)}"
            logger.error(error_msg)
            return jsonify({"error": error_msg}), 500
        ...
        metadata = {
            ...
            "source_url": ZARR_BASE_URL,
            "resolution": TARGET_RESOLUTION_PATH,
            "timestamp": timestamp
        }
        try:
            root.attrs["metadata"] = metadata
            logger.info("Saved metadata to Zarr group")
        except Exception as meta_error:
            error_msg = f"Failed to save metadata: {str(meta_error)}"
            logger.error(error_msg)
            return jsonify({"error": error_msg}), 500
        
        # Get original chunks from target array and adjust for 4D data
        original_chunks = target_zarr_array.chunks
        # Remove the channel dimension from chunks since we're saving each channel separately
        adjusted_chunks = (1, original_chunks[2], original_chunks[3], original_chunks[4])
        logger.info(f"Original chunks: {original_chunks}")
        logger.info(f"Adjusted chunks for 4D data: {adjusted_chunks}")
        
        # Save each channel
        for channel_idx, channel_info in default_channels.items():
            try:
                logger.info(f"Processing channel {channel_idx} ({channel_info['name']})...")
                # Get channel data
                channel_data = target_zarr_array[:, channel_idx, :, :, :]
                logger.info(f"Channel {channel_idx} data shape: {channel_data.shape}")
                
                # Create dataset for this channel
                channel_group = root.create_group(str(channel_idx))
                channel_group.attrs["name"] = channel_info["name"]
                channel_group.attrs["color"] = channel_info["color"]
                channel_group.attrs["window"] = channel_info["window"]
                
                # Save the data using adjusted chunks
                logger.info(f"Saving channel {channel_idx} data...")
                channel_group.create_dataset(
                    "data",
                    data=channel_data,
                    chunks=adjusted_chunks,
                    compressor=pyzarr.Blosc(cname='lz4', clevel=5, shuffle=pyzarr.Blosc.SHUFFLE)
                )
                logger.info(f"Successfully saved channel {channel_idx}")
            except Exception as channel_error:
                error_msg = f"Error processing channel {channel_idx}: {str(channel_error)}"
                logger.error(error_msg)
                return jsonify({"error": error_msg}), 500
        
        logger.info("Successfully saved all channels")
        return jsonify({
            "success": True,
            "message": f"Default channels saved successfully to {output_filename}",
            "path": str(output_path)
        })
        
    except Exception as e:
        error_msg = f"Unexpected error in save_default_channels: {str(e)}"
